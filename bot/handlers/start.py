...
async def cmd_ticker(message: Message, command: CommandObject):
    ...
    try:
        change_pct = f"{float(price_change_24h) * 100:+.2f}%" if price_change_24h is not None else "N/A"
    except (ValueError, TypeError):
        change_pct = "N/A"

    funding_rate = info.get('fundingRate')
    try:
        funding_pct = f"{float(funding_rate) * 100:.4f}%" if funding_rate is not None else "N/A"
    except (ValueError, TypeError):
        funding_pct = "N/A"
    ...
    if mark:
        lines.append(f"Mark: <code>{mark}</code>  |  Index: <code>{index or 'N/A'}</code>")
    ...
    if bid and ask:
        ...
        if bid_vol or ask_vol:
            lines.append(f"Bid vol: <code>{bid_vol or '—'}</code>  |  Ask vol: <code>{ask_vol or '—'}</code>")

    high_24 = info.get('highPrice24h') or ticker.get('high')
    low_24 = info.get('lowPrice24h') or ticker.get('low')
    if high_24 or low_24:
        lines.append(f"24h High: <code>{high_24 or '—'}</code>  |  Low: <code>{low_24 or '—'}</code>")
    ...
    if turnover or volume:
        lines.append(f"Turnover: <code>{turnover or '—'}</code>  |  Vol: <code>{volume or '—'}</code>")

    oi = info.get('openInterest')
    oi_val = info.get('openInterestValue')
    if oi:
        lines.append(f"OI: <code>{oi}</code>" + (f"  |  OI Val: <code>{oi_val}</code>" if oi_val else ""))

    lines.append(f"Funding: <code>{funding_pct}</code>  |  Next: <code>{next_funding}</code>")
